...
class Styles {
    ...
    for(let i = 0; i < arr.length; ++i) {
      ...
      switch(fillObj.type) {
        ...
        case 'GRADIENT_LINEAR':
          fill = this.processGradient('linear', '90deg', fillObj.gradientStops);
          blendMode = this.processBlendMode(i, fillObj.blendMode);
          layer += fill + " " + blendMode;
          break;
      }
  ...
  processStroke(strokeWeight, strokes, stokeAlign) {
    ...
    strokes.forEach((strokeObject) => {
      let borderString;
      if (strokeObject.type === "NORMAL") borderString = 
        `border: ${strokeWeight}px ${strokeObject.type.toLowerCase()} rgba(r:${strokeObject.color.r}, g:${strokeObject.color.g}, b:${strokeObject.color.b}, a:${strokeObject.color.a});`
      if (strokeObject.type === "GRADIENT_LINEAR") borderString = 
        this.processGradient('linear', '90deg', strokeObject.gradientStops);
      res.push(borderString)
    })
    return res.join(" ")
  }
